{{--
    Reusable thumbnail grid for one storage list (Azure or local). Each entry is
    a plain array (id, url, original_name) built by HomeController, not an Eloquent model.
--}}
@if ($images->isEmpty())
    ...
    <ul class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        @foreach ($images as $image)
            ...
            <li class="group relative overflow-hidden rounded-lg border border-gray-200" data-aos="fade">
                ...
                {{-- data-confirm shows a confirm() prompt before submitting (see resources/js/app.js) --}}
